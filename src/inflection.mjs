...
export default class Inflection {
  ...
  static titleOfId(id) {
    ...
    let title = info.id ? info.name : Table.titleOfId(id);
  ...
  static #parseDpdInfDatum(row, opts = {}) {
    ...
    let { pattern, inflections, like } = opts;
    const dbg = 0;
    const dbgv = 0;
    let dbgmsg;
    let dbgvmsg;
      ...
      case 'voc':
        {
          ...
          for (let i = 2; i < keys.length; i += 2) {
            ...
            suffixes.forEach((sfx, j) => {
              ...
              dbg &&
                console.log(msg, '[1]info', JSON.stringify(info));
            }); // suffixes
          }
        }
        break;
      ...
      let agg = Inflection.#KEYS.forEach((k) => {
        ...
        if (akv instanceof Array) {
          ...
          if (akv == null) {
            ...
            a[k] = akv < ikv ? [akv, ikv].flat() : [ikv, akv].flat();
          }
        }
      });
      return a;
