...
import { Dictionary, Inflection, Pali, Table } from '../main.mjs';
...
typeof describe === 'function' &&
  describe('inflection', () => {
    ...
    it('matchesWord() a/ā', () => {
      ...
      should(infs[0].matchesWord('dhamma', { stem: 'dhamm' })).equal(
        true,
      );
      should(infs[0].matchesWord('dhamma', { stem: 'dham' })).equal(
        false,
      );
      ...
      should(infs[0].matchesWord('dhamma', { nbr: 'sg' })).equal(
        true,
      );
      should(infs[0].matchesWord('dhamma', { nbr: 'pl' })).equal(
        false,
      );
      should(infs[1].matchesWord('dhammā', { nbr: 'sg' })).equal(
        false,
      );
      should(infs[1].matchesWord('dhammā', { nbr: 'pl' })).equal(
        true,
      );

      let dhamma = Inflection.TABLE.filter((inf) =>
        inf.matchesWord('dhamma', { stem: 'dhamm' }),
      );
      ...
      should.deepEqual(
        dhamma.rows.map((row) => row.pat),
        ['a nt', 'a masc', 'ā fem'],
      );
      should.deepEqual(
        dhamma.rows.map((row) => row.like),
        ['citta', 'dhamma', 'vedanā'],
      );
    });
    it('matchesWord() dhammānaṁ ', () => {
      ...
      let dhammanam = infs.filter((inf) =>
        inf.matchesWord('dhammānaṁ'),
      );
    ...
    it('matchesWord() i/ī devī', () => {
      ...
      let devi = infs.filter((inf) =>
        inf.matchesWord('devī', { stem }),
      );
      should.deepEqual(devi, [infs[3]]);
      let devihi = infs.filter((inf) =>
        inf.matchesWord('devīhi', { stem }),
      );
    ...
    it('matchesWord() i/ī akkhi', () => {
      ...
      let akkhi = infs.filter((inf) =>
        inf.matchesWord('akkhī', { stem }),
      );
      should.deepEqual(akkhi, [infs[3]]);
      let akkhihi = infs.filter((inf) =>
        inf.matchesWord('akkhīhi', { stem }),
      );
    ...
    it('select() dhamma', () => {
      const msg = 'test.inflection@147';
      const dbg = 0;
      let infDhamma = Inflection.select((inf) =>
        inf.matchesWord('dhamma'),
      );
    ...
    it('select() dhammānaṁd ', () => {
      ...
      let stem = 'dhamm';
      // ṃ
      let infDhammanam = Inflection.select((inf) =>
        inf.matchesWord('dhammānaṁ', { stem }),
      );
    ...
    it('attribute() number', () => {
      ...
      should(attr('number')).properties({
        id: 'nbr',
        type: 'attribute',
      });
      should(attr('sg')).properties({ id: 'sg', type: 'number' });
      should(attr('singular')).properties({
        id: 'sg',
        type: 'number',
      });
    ...
    it('parseDpdInflection()', () => {
      ...
      let { pattern, like, comps, srcTable, inflections } =
